...
Line.prototype.draw = function () {
	...
	this.points.map(function (point) {
		return ctx.lineTo(point.sx, point.sy);
	});
};
function anim() {
	...
	ctx.beginPath();
	pong.points.map(function (point) {
		return point.setScreen();
	});
	pong.lines.map(function (line) {
		return line.draw();
	});
	ctx.stroke();
...
function updatePong() {
	...
	if (pong.ball.respawnTime === 0) {
		...
		if (pong.computer.y > b1) pong.computer.y -= opts.playerSpeed;else if (pong.computer.y + pong.paddle.height < B1) pong.computer.y += opts.playerSpeed;

		if (pong.computer.y < 0) pong.computer.y = 0;else if (pong.computer.y + pong.paddle.height > 1) pong.computer.y = 1 - pong.paddle.height;
		...
		if (pong.ball.vx < 0) {
			...
			if (preva1 > A2 && a1 < A2 && B1 > b2 && b1 < B2) collidePaddle(b2);else if (a1 < 0) resetPongBall(1);
		} else {

			if (preva1 + pong.ball.width < a3 && A1 > a3 && B1 > b3 && b1 < B3) collidePaddle(b3);else if (A1 > 1) resetPongBall(-1);
		}
...
function init() {
	...
	for (var i = 0; i < lineData.length; ++i) {
		lines.push(new Line(lineData[i].map(function (index) {
			return points[index];
		})));
	}for (var i = 0; i < 4 * 3; ++i) {
		pong.points.push(new Point());
	}for (var i = 0; i < 3; ++i) {
		pong.lines.push(new Line([pong.points[i * 4], pong.points[i * 4 + 1], pong.points[i * 4 + 2], pong.points[i * 4 + 3], pong.points[i * 4]]));
	}anim();
}
...
window.addEventListener('keydown', function (e) {
	...
	if (e.keyCode === 32) opts.showDots = !opts.showDots;
...
window.addEventListener('keyup', function (e) {
	...
	// [ A, down, S, J, right ]
	if ([65, 40, 83, 74, 39].indexOf(e.keyCode) !== -1) pong.controls.down = false;

	// [ W, D, up, K, left ]
	if ([87, 68, 38, 75, 37].indexOf(e.keyCode) !== -1) pong.controls.up = false;
...
// inputs
2.5, -0.8, .3, 3.2, -0.8, .3, 2.5, -0.5, .3, 3.2, -0.5, .3, 2.5, -0.2, .3, 3.2, -0.2, .3, 3.2, 0.3, .3, 2.5, 0.3, .3, 2.5, 0.6, .3, 3.2, 0.6, .3, 2.8, 0.9, .3, 3.2, 0.9, .3, 3.2, 1.2, .3, 2.8, 1.2, .3];
lineData = [
// computer
...
// keyboard
[31, 30, 29, 28, 31, 35, 34, 33, 32, 35], [34, 30], [33, 29], [32, 28], [40, 41, 42, 43, 40], [36, 37, 38, 39, 36],
// tower
[44, 45, 46, 47, 44, 48, 49, 50, 51, 48], [51, 47], [50, 46], [49, 45], [52, 53], [54, 55], [56, 57, 58, 59, 56], [60, 61], [62, 63, 64, 65, 62]];
